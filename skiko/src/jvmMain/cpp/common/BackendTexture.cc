...
// side then gets a NULL pointer and throws the documented "Metal not available on this build?" exception,
// instead of an UnsatisfiedLinkError on a missing symbol. Mirrors the Kotlin/Native binding.
...
// Always defined for the same reason as _1nMakeMetal above (Direct3D-less builds throw the documented
// "Direct3D not available on this build?" exception rather than an UnsatisfiedLinkError).
